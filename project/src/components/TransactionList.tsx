  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-navy-900">
      ...
      <div className="bg-navy-800 border-b border-navy-600 px-4 pt-4 pb-0">
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`rounded-xl p-2 transition-all ${showSearch ? 'bg-cyan-400 text-gray-900' : 'bg-white/10 text-white'}`}
            >
        ...
        {showSearch && (
          <div className="mb-3 flex items-center gap-2 rounded-2xl bg-white/10 border border-white/20 px-4 py-2">
      ...
      {filtered.length > 0 && (
        <div className="flex gap-2 px-4 py-2">
          <div className="flex flex-1 items-center gap-2 rounded-2xl bg-green-900/40 px-3 py-2">
            <TrendingUp size={14} className="text-green-400" />
            <div>
              <p className="text-[9px] font-black uppercase text-green-400">In</p>
              <p className="text-sm font-black text-green-300">{fmt(totalIncome)}</p>
            </div>
          </div>
          <div className="flex flex-1 items-center gap-2 rounded-2xl bg-red-900/40 px-3 py-2">
            <TrendingDown size={14} className="text-red-400" />
            <div>
              <p className="text-[9px] font-black uppercase text-red-400">Out</p>
              <p className="text-sm font-black text-red-300">{fmt(totalExpense)}</p>
            </div>
          </div>
          <div className="flex flex-1 items-center gap-2 rounded-2xl bg-white/10 px-3 py-2">
            ...
            <div>
              <p className="text-[9px] font-black uppercase text-slate-300">Net</p>
              <p className={`text-sm font-black ${totalIncome - totalExpense >= 0 ? 'text-green-300' : 'text-red-300'}`}>
            ...
            {(filtered ?? []).map(t => {
              ...
              return (
                <div
                  ...
                  style={{ background: 'linear-gradient(145deg, #0f1f3d 0%, #0a1628 100%)', border: '1px solid rgba(0,229,255,0.1)' }}
                >
                  <div className="flex items-start gap-3">
                    <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl text-xl ${
                      t.type === 'income' ? 'bg-green-900/50' : 'bg-red-900/50'
                    }`}>
                      {cat.emoji}
                    </div>

                    <div className="flex-1 min-w-0">
                      {/* FIX: was text-white on bg-white — now text-white on dark card */}
                      ...
                      <div className="mt-0.5 flex flex-wrap items-center gap-1">
                        ...
                        <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-black uppercase ${
                          ...
                          {(cat.label as Record<string, string>)[lang] ?? (cat.label as Record<string, string>)['en']}
                        </span>
                      </div>
                      {t.voice_transcript && (
                        <p className="mt-1 text-[10px] text-cyan-400 italic truncate">🎤 {t.voice_transcript}</p>
                      )}
                    </div>

                    <div className="flex flex-col items-end gap-1 shrink-0">
                      <p className={`text-base font-black ${t.type === 'income' ? 'text-green-400' : 'text-red-400'}`}>
                        {t.type === 'income' ? '+' : '-'}{fmt(t.amount)}
                      </p>
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => shareOnWhatsApp(t)}
                          className="rounded-lg p-1.5 text-[#25D366] active:bg-green-900/30"
                        >
                          <WhatsAppIcon />
                        </button>
                        <button
                          onClick={() => deleteTransaction(t.id)}
                          className="rounded-lg p-1.5 text-slate-500 active:text-red-400"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
