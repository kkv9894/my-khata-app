  ...
  return (
    <div className="space-y-5 p-4 pb-32">
      <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
        <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-gray-400">Select Month</p>
        <select value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)} className="w-full rounded-xl bg-gray-50 px-4 py-3 font-bold text-gray-800 outline-none">
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-[2rem] border border-gray-100 bg-white p-5 shadow-sm">
              <div className="mb-2 flex items-center gap-2"><TrendingUp size={14} className="text-green-600" /><p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Income</p></div>
              <span className="text-xl font-black text-green-600">₹{totalIncome.toLocaleString('hi-IN')}</span>
            </div>
            <div className="rounded-[2rem] border border-gray-100 bg-white p-5 shadow-sm">
              <div className="mb-2 flex items-center gap-2"><TrendingDown size={14} className="text-red-600" /><p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Expense</p></div>
          ...
          {topCategories.length > 0 && (
            <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
              <p className="mb-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Top Expense Categories</p>
                  ...
                  return (
                    <div key={category}>
                      <div className="mb-1 flex items-center justify-between">
                        ...
                        <span className="text-sm font-black text-gray-900">₹{amount.toLocaleString('hi-IN')} <span className="font-normal text-gray-400">({percent}%)</span></span>
                      </div>
                      <div className="h-2 overflow-hidden rounded-full bg-gray-100"><div className="h-full rounded-full bg-black" style={{ width: `${percent}%` }} /></div>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="space-y-3">
            <p className="px-1 text-[10px] font-black uppercase tracking-widest text-gray-400">Export and Share</p>
            ...
            <button onClick={downloadCSV} disabled={downloading || transactions.length === 0} className="flex w-full items-center justify-center gap-3 rounded-2xl bg-gray-100 p-4 font-bold text-gray-800 disabled:opacity-40"><FileDown size={20} />Download CSV</button>
          ...
          {transactions.length === 0 && (
            <div className="py-8 text-center text-gray-400"><Wallet size={40} className="mx-auto mb-3 opacity-30" /><p className="text-sm font-bold">No transactions this month</p></div>
          )}
        </>
      )}
    </div>
